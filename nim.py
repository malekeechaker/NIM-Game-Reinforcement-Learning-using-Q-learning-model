...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        if (state_tuple,action) in self.q:#check if state/action pair exist in self.q
            return self.q.get((state_tuple,action))
        return 0 #If no Q-value for the state/action pair exists in self.q, then the function should return 0.
    ...
    def choose_action(self, state, epsilon=True):
        ...
        bestReward = 0
        for action in actions:
            currentReward = self.get_q_value(state,action)
            if bestAction is None or currentReward > bestReward:
                bestAction = action
                bestReward = currentReward
        if epsilon:
            weights = [(1-self.epsilon) if action == bestAction else (self.epsilon/(len(actions)-1)) for action in actions ]
